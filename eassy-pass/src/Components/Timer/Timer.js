...
class Timer extends Component{
    ...
    componentDidMount = () => {
     ...
     checkout = () => {
         ...
         setInterval(() => {
            ...
            copyState.expire = !this.state.expire
            this.setState(copyState)
         }, 500);
      }
    ...
    render(){
        return(
            <div className="timer">
                 <button onClick={this.checkout}className="hidden-button" style={ {color: "#1be620"}}>xxx</button>
                <h1>
                    {this.state.expire ? 
                    <div>
                        <h1 className="expire" >انتهاء التصريح</h1>
                    </div>
                    :
                    this.state.show ? this.state.date.toLocaleTimeString([],{ hour: '2-digit', minute: '2-digit' }) :this.state.word ? this.state.word:this.state.words[new Date().getDay()]
                    }
                </h1>
            </div>
        )
